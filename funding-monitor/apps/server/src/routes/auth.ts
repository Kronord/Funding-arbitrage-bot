  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!email || !password) {
        return res
          .status(400)
          .json({ ok: false, error: "Email та пароль обовʼязкові" });
      }
      ...
      if (password.length < 8) {
        return res
          .status(400)
          .json({ ok: false, error: "Пароль мінімум 8 символів" });
      }
      ...
      if (existing) {
        return res
          .status(409)
          .json({ ok: false, error: "Email вже зареєстрований" });
      }
...
router.post("/login", loginLimiter, async (req: Request, res: Response) => {
  try {
    ...
    if (!user || !user.passwordHash) {
      return res
        .status(401)
        .json({ ok: false, error: "Невірний email або пароль" });
    }
...
router.post("/refresh", async (req: Request, res: Response) => {
  try {
    ...
    if (!refreshToken) {
      return res
        .status(401)
        .json({ ok: false, error: "Refresh token відсутній" });
    }
    ...
    try {
      ...
      return res
        .status(401)
        .json({ ok: false, error: "Невалідний refresh token" });
    }
    ...
    if (!session) {
      return res
        .status(401)
        .json({ ok: false, error: "Сесія не знайдена або закінчилась" });
    }
...
router.get("/me", requireAuth, async (req: Request, res: Response) => {
  ...
  if (!user)
    return res.status(404).json({ ok: false, error: "Юзер не знайдений" });
...
router.patch("/profile", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (!name?.trim()) {
      return res
        .status(400)
        .json({ ok: false, error: "Імʼя не може бути порожнім" });
    }
...
router.patch("/password", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (!user?.passwordHash) {
      return res
        .status(400)
        .json({ ok: false, error: "Акаунт без пароля (Google OAuth)" });
    }
    ...
    if (!valid) {
      return res
        .status(401)
        .json({ ok: false, error: "Невірний поточний пароль" });
    }
