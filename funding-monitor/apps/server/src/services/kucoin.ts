...
async function getOrderBook(
  ...
  try {
    const [spotOB, futOB] = await Promise.all([
      axios.get(
        `https://api.kucoin.com/api/v1/market/orderbook/level2_20?symbol=${spotSymbol}`,
      ),
      axios.get(
        `https://api-futures.kucoin.com/api/v1/level2/depth20?symbol=${futSymbol}`,
      ),
    ]);
    return {
...
export async function fetchKucoinFunding(
  ...
  data.spot.forEach((s) => {
    ...
    const is1000 =
      !contractMap.has(futSymbol) && contractMap.has(`1000${coin}USDTM`);
...
export async function fetchCoinDetails(coin: string): Promise<any> {
  ...
  const [contractRes, spotRes] = await Promise.all([
    axios
      .get(`https://api-futures.kucoin.com/api/v1/contracts/${futSymbol}`)
      .catch(() =>
        axios.get(`https://api-futures.kucoin.com/api/v1/contracts/${alt1000}`),
      ),
    axios.get(
...
export function calcSpreadByEntryPrice(
  ...
  const netEntry = parseFloat((basisEntry - TOTAL_FEE * 100).toFixed(3));
