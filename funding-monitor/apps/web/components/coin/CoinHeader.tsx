...
function Stat({ label, value, cls = '' }: { label: string; value: string; cls?: string }) {
  return (
    <div className="bg-surface border border-border rounded-lg px-4 py-3">
      <div className="text-[10px] font-mono uppercase tracking-widest text-text-dim mb-1">{label}</div>
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h2 className="text-2xl font-bold text-[#cdd9e5] font-mono">{d.coin}<span className="text-text-muted font-normal text-base">/USDT</span></h2>
          <div className="text-xs text-text-dim font-mono mt-0.5">{d.symbol} · KuCoin Futures</div>
        </div>
        <div className="ml-auto text-right">
          <div className="text-2xl font-mono font-bold text-[#cdd9e5]">${d.markPrice.toFixed(6)}</div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
        ...
        <Stat label="Через"      value={getCountdown(d.nextFundingTs)} cls={urgent ? 'text-red' : 'text-[#cdd9e5]'} />
        <Stat label="Index Price" value={`$${d.indexPrice.toFixed(6)}`} />
        <Stat label="Об'єм 24г"  value={`$${(d.turnover24h / 1e6).toFixed(2)}M`} />
