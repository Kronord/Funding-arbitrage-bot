...
export default function EntryCalculator({
  ...
  return (
    <div className="bg-surface border border-border rounded-lg p-5">
      <h3 className="text-sm font-bold text-[#cdd9e5] mb-4">
        Калькулятор входу
      </h3>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        {[
          {
            label: "Ціна входу Спот",
            value: spotPrice,
            set: setSpotPrice,
            placeholder: details.markPrice.toFixed(6),
          },
          {
            label: "Ціна входу Ф'юч",
            value: futPrice,
            set: setFutPrice,
            placeholder: details.markPrice.toFixed(6),
          },
      ...
      {result && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {[
            {
              label: "Avg Спот (ask)",
              value: <Val v={result.avgSpotAsk} suffix="" />,
            },
            {
              label: "Avg Ф'юч (bid)",
            ...
            <div
              key={s.label}
              className="bg-bg border border-border rounded px-3 py-2.5"
            >
      ...
      <p className="text-[10px] font-mono text-text-dim mt-4">
        * Спред (стакан) — середня ціна по відкритих ордерах на поточний об'єм ·
        Спред (вхід) — по вашим цінам входу
      </p>
    </div>
  );
}
