...
export default function FundingChart({ coin }: { coin: string }) {
  ...
  return (
    <div className="bg-surface border border-border rounded-lg p-5">
      <h3 className="text-sm font-bold text-[#cdd9e5] mb-4">Історія фандингу</h3>
      <div className="h-40 flex items-center justify-center text-text-dim font-mono text-xs">
        Історія фандингу ще завантажується...
      </div>
    </div>
  );
}
      ...
      {summary && (
        <div className="grid grid-cols-4 gap-3 mb-4">
          {[
            {
              label: "Сума фандингу",
              value: `${summary.totalRate > 0 ? "+" : ""}${summary.totalRate}%`,
              cls: summary.totalRate > 0 ? "text-green" : "text-red",
            },
            ...
            >
              ...
              <div className={`text-sm font-mono font-bold ${s.cls}`}>
                {s.value}
              </div>
            </div>
          ))}
          ...
          <BarChart data={data}>
            ...
            <Bar dataKey="rate" radius={[2, 2, 0, 0]}>
              {data.map((entry, i) => (
                <Cell
                  key={i}
                  fill={entry.rate >= 0 ? "#39d353" : "#f85149"}
                  fillOpacity={0.8}
                />
              ))}
            </Bar>
        ...
        <table className="w-full text-[11px] font-mono">
          <thead>
            <tr className="border-b border-border">
              <th className="text-left text-text-dim py-1.5 font-normal">
                Час
              </th>
              <th className="text-right text-text-dim py-1.5 font-normal">
                Фандинг
              </th>
            </tr>
          </thead>
          <tbody>
            {[...data].reverse().map((h, i) => (
              ...
              >
                ...
                <td
                  className={`text-right py-1.5 font-bold ${h.rate >= 0 ? "text-green" : "text-red"}`}
                >
                  {h.rate >= 0 ? "+" : ""}
                  {h.rate}%
                </td>
