  ...
  return (
    <div className="bg-surface border border-border rounded-lg p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-bold text-[#cdd9e5]">Сума виплат фандингу</h3>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            <div key={s.label} className="bg-bg border border-border rounded-lg px-4 py-3">
              <div className="text-[10px] font-mono uppercase tracking-widest text-text-dim mb-1">{s.label}</div>
              <div className={`text-lg font-mono font-bold ${s.cls}`}>{s.value}</div>
              <div className="text-[10px] font-mono text-text-dim mt-1">{s.desc}</div>
            </div>
          ))}
        </div>
      ) : null}
